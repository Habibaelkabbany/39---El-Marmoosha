...
class UserServiceTest {
    ...
    void testAddUser_EdgeCase() {
        ...
        when(userRepository.addUser(emptyUser)).thenReturn(emptyUser);
        assertEquals("", userService.addUser(emptyUser).getName());
    }
    ...
    void testAddOrderToUser_Positive() {
        assertDoesNotThrow(() -> userService.addOrderToUser(user.getId()));
    }

    @Test
    void testAddOrderToUser_EdgeCase() {
        assertDoesNotThrow(() -> userService.addOrderToUser(new UUID(0, 0)));
    }
