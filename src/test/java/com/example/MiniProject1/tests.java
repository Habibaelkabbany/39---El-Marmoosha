...
//    @Test void testAddUser_EdgeCase() { User emptyUser = new User(null, "", new ArrayList<>()); when(userRepository.addUser(emptyUser)).thenReturn(emptyUser); assertEquals("", userService.addUser(emptyUser).getName()); }
//
//    // TEST: getUsers()
//    @Test void testGetUsers_Positive() { when(userRepository.getUsers()).thenReturn(List.of(user)); assertFalse(userService.getUsers().isEmpty()); }
//    @Test void testGetUsers_Negative() { when(userRepository.getUsers()).thenReturn(Collections.emptyList()); assertTrue(userService.getUsers().isEmpty()); }
//    @Test void testGetUsers_EdgeCase() { when(userRepository.getUsers()).thenReturn(Collections.emptyList()); assertEquals(0, userService.getUsers().size()); }
//
//    // TEST: getUserById()
...
//
//    // TEST: addOrderToUser()
//    @Test void testAddOrderToUser_Positive() { doNothing().when(userRepository).addOrderToUser(user.getId(), order); assertDoesNotThrow(() -> userService.addOrderToUser(user.getId(), order)); }
//    @Test void testAddOrderToUser_Negative() { doThrow(new RuntimeException("User not found")).when(userRepository).addOrderToUser(any(), any()); assertThrows(RuntimeException.class, () -> userService.addOrderToUser(UUID.randomUUID(), order)); }
//    @Test void testAddOrderToUser_EdgeCase() { doNothing().when(userRepository).addOrderToUser(new UUID(0, 0), order); assertDoesNotThrow(() -> userService.addOrderToUser(new UUID(0, 0), order)); }
//
