...
//    @Test void testAddUser_EdgeCase() { User emptyUser = new User(null, "", new ArrayList<>()); when(userRepository.addUser(emptyUser)).thenReturn(emptyUser); assertEquals("", userService.addUser(emptyUser).getName()); }
    ...
    void testGetUsers_Positive() {
        when(userRepository.getUsers()).thenReturn(new ArrayList<>(Arrays.asList(user)));
        assertFalse(userService.getUsers().isEmpty(), "Expected non-empty user list.");
    }

    @Test
    void testGetUsers_EmptyList() {
        when(userRepository.getUsers()).thenReturn(new ArrayList<>());
        assertTrue(userService.getUsers().isEmpty(), "Expected empty user list.");
    }

    @Test
    void testGetUsers_Negative() {
        when(userRepository.getUsers()).thenThrow(new RuntimeException("Database error"));
        assertThrows(RuntimeException.class, () -> userService.getUsers(), "Expected exception when repository fails.");
    }

    @Test
    void testGetUserById_Negative() {
        when(userRepository.getUserById(any())).thenReturn(null);
        assertThrows(RuntimeException.class, () -> userService.getUserById(UUID.randomUUID()), "Expected exception when user is not found.");
    }

    @Test
    void testGetUserById_EdgeCase() {
        when(userRepository.getUserById(new UUID(0, 0))).thenReturn(null);
        assertThrows(RuntimeException.class, () -> userService.getUserById(new UUID(0, 0)), "Expected exception for empty UUID.");
    }
